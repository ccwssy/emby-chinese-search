...
dotnet build -c Release EmbyChineseSearch.sln --nologo

# 3. 复制 ILRepack 依赖（SQLitePCLRawEx.core 不在 NuGet 中，在容器 /system/）
docker cp emby:/system/SQLitePCLRawEx.core.dll EmbyChineseSearch/bin/Release/net6.0/

# 4. 合并依赖到单 DLL（必须，否则运行时缺少 0Harmony/TinyPinyin 等）
...
> ⚠️ `libs/publicized/` 中的 DLL 只是编译时引用（`<Private>False</Private>`），不会进入输出目录，构建好的插件不依赖它们。
> ⚠️ ILRepack 合并 4 个依赖 DLL 后生成的合并 DLL 约 6.8MB，放入 `/config/plugins/` 时保持文件名 `EmbyChineseSearch.dll`。
